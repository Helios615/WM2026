...
export default function App() {
  ...
  const [oddsApiKey, setOddsApiKey] = useState(() => {
    return localStorage.getItem('wc_odds_apikey') || '';
  });
  const [liveMatches, setLiveMatches] = useState<any[]>(() => {
    const saved = localStorage.getItem('wc_live_matches');
  ...
  useEffect(() => {
    localStorage.setItem('wc_odds_apikey', oddsApiKey);
  }, [oddsApiKey]);

  useEffect(() => {
    localStorage.setItem('wc_live_matches', JSON.stringify(liveMatches));
  }, [liveMatches]);
  ...
  const translateTeam = (englishName: string) => {
    const team = WORLD_CUP_TEAMS.find(t => 
      t.englishName.toLowerCase() === englishName.toLowerCase() || 
      t.name === englishName
    );
    if (team) {
      return { nameCn: team.name, flag: team.flag };
    }
    return { nameCn: englishName, flag: '⚽' };
  };

  const syncLiveOdds = async () => {
    if (!oddsApiKey.trim()) {
      setSyncError('请输入 API Key！');
      return;
    }
    setSyncing(true);
    setSyncError(null);
    try {
      const url = `https://api.the-odds-api.com/v4/sports/soccer_fifa_world_cup/odds/?apiKey=${oddsApiKey.trim()}&regions=eu,us&markets=h2h,spreads,totals&oddsFormat=decimal`;
      const response = await fetch(url);
      if (!response.ok) {
        if (response.status === 401) {
          throw new Error('API Key 无效或额度超限！');
        }
      ...
      const parsedMatches = data.map((match: any) => {
        const homeTranslate = translateTeam(match.home_team);
        const awayTranslate = translateTeam(match.away_team);
        
        let bookmaker = match.bookmakers.find((b: any) => b.key.toLowerCase() === 'bet365' || b.key.toLowerCase() === 'bet365_au');
        if (!bookmaker && match.bookmakers.length > 0) {
          bookmaker = match.bookmakers[0];
        }
        
        let h2h: any = undefined;
        let spreads: any = undefined;
        let totals: any = undefined;
        let bookmakerTitle = '无数据';
        ...
        if (bookmaker) {
          ...
          if (h2hMarket && h2hMarket.outcomes) {
            ...
            const draw = h2hMarket.outcomes.find((o: any) => o.name.toLowerCase() === 'draw')?.price;
            if (home && away && draw) {
              h2h = { home, draw, away };
            }
          }
          ...
          if (spreadsMarket && spreadsMarket.outcomes && spreadsMarket.outcomes.length >= 2) {
            const homeOutcome = spreadsMarket.outcomes.find((o: any) => o.name === match.home_team);
            const awayOutcome = spreadsMarket.outcomes.find((o: any) => o.name === match.away_team);
        ...
        return {
          ...
          homeTeamCn: homeTranslate.nameCn,
          homeTeamFlag: homeTranslate.flag,
          awayTeam: match.away_team,
          awayTeamCn: awayTranslate.nameCn,
          awayTeamFlag: awayTranslate.flag,
          bookmakerTitle,
          h2h,
          spreads,
          totals
      ...
      setLiveMatches(parsedMatches);
      alert(`同步成功！已获取 ${parsedMatches.length} 场比赛的实时赔率（包含独赢、让球、大小球）。`);
    } catch (err: any) {
      console.error(err);
      setSyncError(err.message || '获取赔率失败，请检查网络或 API Key！');
    } finally {
      setSyncing(false);
    }
  };

  const selectLiveOdds = (
    match: any, 
    ...
    setBetHomeTeam(`${match.homeTeamCn} ${match.homeTeamFlag}`);
    setBetAwayTeam(`${match.awayTeamCn} ${match.awayTeamFlag}`);
    ...
    if (marketType === 'h2h') {
      playType = '独赢 (主客和)';
      if (outcome === 'home') detail = `主胜 - ${match.homeTeamCn}`;
      if (outcome === 'away') detail = `客胜 - ${match.awayTeamCn}`;
      if (outcome === 'draw') detail = `平局`;
    } else if (marketType === 'spreads') {
      playType = '让球盘 (亚洲盘)';
      const formattedPoint = point && point > 0 ? `+${point}` : `${point}`;
      if (outcome === 'home') detail = `${match.homeTeamCn} (${formattedPoint})`;
      if (outcome === 'away') detail = `${match.awayTeamCn} (${formattedPoint})`;
    } else if (marketType === 'totals') {
      playType = '大小盘 (进球数)';
      if (outcome === 'over') detail = `大 ${point}`;
      if (outcome === 'under') detail = `小 ${point}`;
    }

    setBetPlayType(playType);
    setBetPlayDetail(detail);
    setBetOdds(price.toString());
    
    const element = document.getElementById('bet-entry-form-header');
    if (element) {
...
alter table wc_transactions disable row level security;`}
        ...
        {activeTab === 'bets' && (
          <div className="space-y-6">
            {/* Bet365 Live Odds Syncer */}
            <div className="glass-panel border border-[var(--primary-glow)]">
              <div className="flex flex-wrap justify-between items-center gap-4 mb-4">
                <div>
                  <h3 className="title-font text-lg font-bold flex items-center gap-2 text-[var(--primary)]">
                    ⚡ Bet365 实时赔率同步
                  </h3>
                  <p className="text-xs text-[var(--text-muted)] mt-1">
                    使用免费的 Odds API 同步最新的世界杯比赛赔率，点击赔率直接填充到下方的录入单中。
                  </p>
                </div>
                <div className="flex gap-2 items-center flex-wrap flex-grow md:flex-initial">
                  <input 
                    type="password"
                    placeholder="在此输入您的 API Key"
                    className="form-input text-xs w-[180px] h-[34px] py-1"
                    value={oddsApiKey}
                    onChange={e => setOddsApiKey(e.target.value)}
                  />
                  <button 
                    type="button" 
                    className="btn btn-primary text-xs py-1.5 h-[34px] px-3" 
                  ...
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer" 
                    className="text-[10px] text-[var(--primary)] underline ml-1 hover:text-amber-400"
                  >
                    免费获取Key
                  </a>
                </div>
              ...
              {syncError && (
                <div className="text-xs text-red-400 bg-red-900/20 p-2 rounded mb-4 flex items-center gap-2">
                  <AlertCircle size={14} /> {syncError}
                </div>
              )}

              {liveMatches.length === 0 ? (
                <div className="text-center py-6 text-xs text-[var(--text-muted)] bg-slate-900/30 rounded border border-dashed border-slate-800">
                  暂无同步数据。输入您的 Odds API Key 并点击“同步赔率”按钮即可同步最新赔率。
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[300px] overflow-y-auto pr-1">
                  {liveMatches.map((m: any) => (
                    <div key={m.id} className="bg-slate-900/60 p-3 rounded-xl border border-slate-800 flex flex-col justify-between gap-3 text-xs">
                      <div className="flex justify-between items-center text-[10px] text-slate-500">
                        <span>🕒 {formatDate(new Date(m.commenceTime).getTime(), 'short')}</span>
                        <span className="bg-slate-800 px-1.5 py-0.5 rounded text-[9px]">数据源: {m.bookmakerTitle}</span>
                      </div>
                      
                      <div className="flex justify-between items-center font-bold text-slate-200">
                        <span className="flex items-center gap-1">
                          <span className="text-base">{m.homeTeamFlag}</span>
                          <span>{m.homeTeamCn}</span>
                        </span>
                        <span className="text-[10px] text-slate-500 font-normal">VS</span>
                        <span className="flex items-center gap-1">
                          <span>{m.awayTeamCn}</span>
                          <span className="text-base">{m.awayTeamFlag}</span>
                        </span>
                      </div>
                      ...
                      <div className="flex flex-col gap-2 border-t border-slate-800/60 pt-2">
                        ...
                        <div className="flex justify-between items-center gap-1.5">
                          <span className="text-[10px] text-slate-500 w-[24px] flex-shrink-0 text-left font-bold">独赢</span>
                          <div className="grid grid-cols-3 gap-1 flex-grow">
                            <button
                              type="button"
                              className="bg-slate-800 hover:bg-[var(--primary-glow)] hover:text-[var(--primary)] border border-slate-700/30 rounded py-1 transition text-center text-[10px] font-bold text-slate-300"
                              onClick={() => selectLiveOdds(m, 'h2h', 'home', m.h2h.home)}
                              disabled={!m.h2h?.home}
                            >
                              主 {m.h2h?.home ? m.h2h.home.toFixed(2) : '-'}
                            </button>
                            <button
                              type="button"
                              className="bg-slate-800 hover:bg-[var(--primary-glow)] hover:text-[var(--primary)] border border-slate-700/30 rounded py-1 transition text-center text-[10px] font-bold text-slate-300"
                              onClick={() => selectLiveOdds(m, 'h2h', 'draw', m.h2h.draw)}
                              disabled={!m.h2h?.draw}
                            >
                              平 {m.h2h?.draw ? m.h2h.draw.toFixed(2) : '-'}
                            </button>
                            <button
                              type="button"
                              className="bg-slate-800 hover:bg-[var(--primary-glow)] hover:text-[var(--primary)] border border-slate-700/30 rounded py-1 transition text-center text-[10px] font-bold text-slate-300"
                              onClick={() => selectLiveOdds(m, 'h2h', 'away', m.h2h.away)}
                              disabled={!m.h2h?.away}
                            >
                              客 {m.h2h?.away ? m.h2h.away.toFixed(2) : '-'}
                            </button>
                          </div>
                        </div>
                        ...
                        {/* Spreads Market */}
                        <div className="flex justify-between items-center gap-1.5">
                          <span className="text-[10px] text-slate-500 w-[24px] flex-shrink-0 text-left font-bold">让球</span>
                          <div className="grid grid-cols-2 gap-1 flex-grow">
                            <button
                              type="button"
                              className="bg-slate-800 hover:bg-[var(--primary-glow)] hover:text-[var(--primary)] border border-slate-700/30 rounded py-1 transition text-center text-[10px] font-bold text-slate-300"
                              onClick={() => selectLiveOdds(m, 'spreads', 'home', m.spreads.home.price, m.spreads.home.point)}
                              disabled={!m.spreads?.home}
                            >
                              {m.spreads?.home 
                                ? `${m.spreads.home.point > 0 ? '+' : ''}${m.spreads.home.point} @${m.spreads.home.price.toFixed(2)}` 
                                : '未开盘'}
                            </button>
                            <button
                              type="button"
                              className="bg-slate-800 hover:bg-[var(--primary-glow)] hover:text-[var(--primary)] border border-slate-700/30 rounded py-1 transition text-center text-[10px] font-bold text-slate-300"
                              onClick={() => selectLiveOdds(m, 'spreads', 'away', m.spreads.away.price, m.spreads.away.point)}
                              disabled={!m.spreads?.away}
                            >
                              {m.spreads?.away 
                                ? `${m.spreads.away.point > 0 ? '+' : ''}${m.spreads.away.point} @${m.spreads.away.price.toFixed(2)}` 
                                : '未开盘'}
                            </button>
                          </div>
                        </div>
                        ...
                        {/* Totals Market */}
                        <div className="flex justify-between items-center gap-1.5">
                          <span className="text-[10px] text-slate-500 w-[24px] flex-shrink-0 text-left font-bold">大小</span>
                          <div className="grid grid-cols-2 gap-1 flex-grow">
                            <button
                              type="button"
                              className="bg-slate-800 hover:bg-[var(--primary-glow)] hover:text-[var(--primary)] border border-slate-700/30 rounded py-1 transition text-center text-[10px] font-bold text-slate-300"
                              onClick={() => selectLiveOdds(m, 'totals', 'over', m.totals.over.price, m.totals.over.point)}
                              disabled={!m.totals?.over}
                            >
                              {m.totals?.over ? `大 ${m.totals.over.point} @${m.totals.over.price.toFixed(2)}` : '未开盘'}
                            </button>
                            <button
                              type="button"
                              className="bg-slate-800 hover:bg-[var(--primary-glow)] hover:text-[var(--primary)] border border-slate-700/30 rounded py-1 transition text-center text-[10px] font-bold text-slate-300"
                              onClick={() => selectLiveOdds(m, 'totals', 'under', m.totals.under.price, m.totals.under.point)}
                              disabled={!m.totals?.under}
                            >
                              {m.totals?.under ? `小 ${m.totals.under.point} @${m.totals.under.price.toFixed(2)}` : '未开盘'}
                            </button>
                          </div>
                        </div>
                      </div>
